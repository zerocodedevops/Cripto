...
interface TransactionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (tx: { coinId: string; type: 'buy' | 'sell'; amount: number; pricePerCoin: number; date: string }) => void;
}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          ...
          >
            ...
            <div className="flex items-center justify-between p-6 border-b border-white/5">
              <h3 className="text-xl font-bold" style={{ color: cryptoTheme.colors.text.primary }}>
                Añadir Transacción
              </h3>
              <button onClick={onClose} className="p-2 hover:bg-white/5 rounded-full transition-colors">
                <X className="w-5 h-5" style={{ color: cryptoTheme.colors.text.secondary }} />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              {/* Asset Select */}
              <div>
                <label htmlFor="tx-coin-id" className="block text-sm font-medium mb-2" style={{ color: cryptoTheme.colors.text.secondary }}>
                  Activo
                </label>
                <select
                  id="tx-coin-id"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label htmlFor="tx-amount" className="block text-sm font-medium mb-2" style={{ color: cryptoTheme.colors.text.secondary }}>
                    Cantidad
                  </label>
                ...
                <div>
                  <label htmlFor="tx-price" className="block text-sm font-medium mb-2" style={{ color: cryptoTheme.colors.text.secondary }}>
                    Precio por Moneda
                  </label>
                  <div className="relative">
                    <input
              ...
              {/* Date */}
              <div>
                <label htmlFor="tx-date" className="block text-sm font-medium mb-2" style={{ color: cryptoTheme.colors.text.secondary }}>
                  Fecha
                </label>
                <div className="relative">
                  <input
            ...
            <button
              ...
              style={{
                background: type === 'buy' ? cryptoTheme.colors.status.bullish : cryptoTheme.colors.status.bearish,
                color: '#fff'
              }}
            >
