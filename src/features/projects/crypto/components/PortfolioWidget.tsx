					...
					<div className="mt-4 space-y-2 max-h-[150px] overflow-y-auto pr-2 custom-scrollbar">
						{holdings.length === 0 ? (
							...
							holdings.map((h) => (
								<div
									key={h.id}
									className="flex justify-between text-sm py-2 border-b border-white/5 last:border-0 hover:bg-white/5 p-2 rounded-lg transition-colors"
								>
									<span className="font-medium capitalize">{h.id}</span>
									<span className="opacity-70">
										{h.amount.toFixed(4)} {h.id.substring(0, 3).toUpperCase()}
									</span>
								</div>
							))
						)}
