  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-6">
        ...
        >
          <ArrowRightLeft className="w-5 h-5" style={{ color: cryptoTheme.colors.primary.electric }} />
        </div>
      ...
      <div className="space-y-4">
        ...
        {/* Rows */}
        <ComparisonRow label="Ranking" valA={`#${dataA?.market_cap_rank}`} valB={`#${dataB?.market_cap_rank}`} />
        <ComparisonRow label="Precio" valA={formatCurrency(dataA?.current_price || 0)} valB={formatCurrency(dataB?.current_price || 0)} highlight />
        <ComparisonRow 
            label="Cap. Mercado" 
        ...
        <ComparisonRow 
            ...
            colorA={dataA?.price_change_percentage_24h && dataA.price_change_percentage_24h >= 0 ? 'text-green-500' : 'text-red-500'}
            colorB={dataB?.price_change_percentage_24h && dataB.price_change_percentage_24h >= 0 ? 'text-green-500' : 'text-red-500'}
        />
      </div>
    </div>
  );
...
function ComparisonRow({ label, valA, valB, highlight = false, colorA, colorB }: { label: string, valA: string, valB: string, highlight?: boolean, colorA?: string, colorB?: string }) {
    return (
        <div className={`grid grid-cols-3 items-center py-3 border-b border-white/5 last:border-0 ${highlight ? 'bg-white/5 rounded-lg px-2 -mx-2' : ''}`}>
            <div className="text-sm opacity-70">{label}</div>
            <div className={`text-center font-bold text-sm ${colorA || ''}`}>{valA}</div>
            <div className={`text-center font-bold text-sm ${colorB || ''}`}>{valB}</div>
        </div>
    )
}
