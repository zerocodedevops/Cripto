...
export function WatchlistWidget() {
  ...
  const watchlistCoins = allCoins?.filter(c => watchlist.includes(c.id)) || [];
  ...
  if (watchlist.length === 0) {
    return (
      ...
      >
        <Star className="w-12 h-12 mb-4" style={{ color: '#fbbf24', fill: '#fbbf24' }} />
        <h3 className="text-xl font-bold mb-2" style={{ color: cryptoTheme.colors.text.primary }}>
          Tu Watchlist
        </h3>
        <p className="text-sm px-4" style={{ color: cryptoTheme.colors.text.tertiary }}>
          Añade criptos favoritas usando la estrella ⭐ en las listas para seguirlas aquí.
        </p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs font-bold px-2 py-1 rounded-full bg-white/5" style={{ color: cryptoTheme.colors.text.secondary }}>
          {watchlist.length}
        </span>
      </div>
        ...
        {watchlistCoins.map((coin) => (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-bold text-sm" style={{ color: cryptoTheme.colors.text.primary }}>
                  {coin.symbol.toUpperCase()}
                </div>
                <div className="text-xs" style={{ color: cryptoTheme.colors.text.tertiary }}>
                  {coin.name}
                </div>
              </div>
            </div>
