...
import type { CryptoMarket, GlobalMarketData, TrendingCoin, MarketChartData } from '../types/crypto';
...
class CoinGeckoClient {
  ...
  constructor() {
    ...
    console.log(`[CoinGecko Client] Using ${USE_REAL_API ? 'real API via serverless' : 'MSW mocks'}`);
  }
  ...
  private async fetch<T>(endpoint: string): Promise<T> {
    ...
    const fullEndpoint = USE_REAL_API 
      ? endpoint.replace('/coins/markets', '/markets')
  ...
  async getCoin(id: string, params: {
  ...
  async getMarketChart(id: string, params: {
    vs_currency?: string;
    days?: string | number;
    interval?: string;
  ...
  async getSimplePrice(ids: string[], params: {
  ...
  async getOHLC(id: string, params: {
    vs_currency?: string;
    days?: string | number;
  } = {}): Promise<number[][]> {
    const defaults = {
