...
import { CryptoMarket as Coin } from '../types/crypto';
  ...
  useEffect(() => {
    const fetchPrices = async () => {
      try {
        ...
        const response = await fetch(`https://api.coingecko.com/api/v3/coins/markets?vs_currency=eur&order=market_cap_desc&per_page=${options.per_page}&page=1&sparkline=false`);
