...
export function PortfolioProvider({ children }: { readonly children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setTransactions([
        { id: '1', coinId: 'bitcoin', type: 'buy', amount: 0.5, pricePerCoin: 45000, date: new Date().toISOString() },
        { id: '2', coinId: 'ethereum', type: 'buy', amount: 10, pricePerCoin: 2500, date: new Date().toISOString() }
      ]);
    }
  }, []);
  ...
  const holdings = useMemo(() => {
    ...
    transactions.forEach(tx => {
      const current = map.get(tx.coinId) || { id: tx.coinId, amount: 0, avgBuyPrice: 0 };
      
      if (tx.type === 'buy') {
        const totalCost = (current.amount * current.avgBuyPrice) + (tx.amount * tx.pricePerCoin);
  ...
  const value = useMemo(() => ({
    ...
    totalInvested: holdings.reduce((acc, h) => acc + (h.amount * h.avgBuyPrice), 0)
