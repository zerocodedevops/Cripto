...
export default function ReviewList({ productId, refreshTrigger }: ReviewListProps) {
  ...
  if (loading) {
      return <div className="text-center py-10 text-slate-500">Cargando opiniones...</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-xl font-bold text-slate-900">Opiniones de Clientes ({reviews.length})</h3>
              ...
              <div>
                ...
                <div className="flex text-yellow-400">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={`star-${i}`} className={`w-4 h-4 ${i < review.rating ? 'fill-current' : 'text-slate-200'}`} />
                  ))}
                </div>
              </div>
            </div>
            <span className="text-xs text-slate-400">
              {review.createdAt?.seconds ? new Date(review.createdAt.seconds * 1000).toLocaleDateString() : 'Reciente'}
            </span>
          </div>
