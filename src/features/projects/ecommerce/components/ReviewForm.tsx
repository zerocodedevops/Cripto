...
import { storage, db } from '../../../../config/firebase'; // Corrected Path (features/projects/ecommerce/components -> ../../../../config)
...
export default function ReviewForm({ productId, onReviewAdded }: ReviewFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (image) {
        const storageRef = ref(storage, `reviews/${productId}/${Date.now()}_${image.name}`);
  ...
  if (!user) {
      return (
          <div className="bg-slate-50 p-6 rounded-2xl text-center border border-slate-200">
              <p className="text-slate-600 mb-2">Inicia sesión para compartir tu experiencia</p>
          </div>
      );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
      <h3 className="text-lg font-bold text-slate-900 mb-4">Escribe una Reseña</h3>

      {/* Rating */}
      <div className="mb-4">
        <label className="block text-sm font-medium text-slate-700 mb-2">Valoración</label>
      ...
      {/* Comment */}
      <div className="mb-4">
        <label htmlFor="comment-input" className="block text-sm font-medium text-slate-700 mb-2">Tu Opinión</label>
        <textarea
          id="comment-input"
          value={comment}
      ...
      <div className="mb-6">
        ...
        {!preview ? (
          ...
          >
            ...
            <span className="text-sm text-slate-500 group-hover:text-indigo-600 font-medium">Subir foto real</span>
            <input 
              ref={fileInputRef}
              type="file" 
              accept="image/*" 
