...
import { Calendar, Clock, MapPin, Scissors, Loader2, CreditCard, ChevronRight } from 'lucide-react';
...
export default function MyBookings() {
    ...
    const BookingCard = ({ booking, isPast }: { booking: any, isPast?: boolean }) => (
        <div className={`bg-slate-900 border rounded-xl overflow-hidden transition-all hover:border-amber-500/30 group ${isPast ? 'border-slate-800 opacity-75' : 'border-slate-700 shadow-lg'}`}>
            <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                    <div>
                        <h3 className="text-lg font-bold text-slate-100 flex items-center gap-2">
                            ...
                            {booking.services?.name || booking.service_id || 'Servicio de Peluquería'}
                        </h3>
                        <p className="text-slate-400 text-sm mt-1">con {booking.stylist?.name || 'Estilista Asignado'}</p>
                    </div>
                    <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border ${booking.status === 'approved'
                            ? 'bg-emerald-900/30 text-emerald-400 border-emerald-800/50'
                            : booking.status === 'pending'
                                ? 'bg-amber-900/30 text-amber-400 border-amber-800/50'
                                : 'bg-red-900/30 text-red-400 border-red-800/50'
                        }`}>
                        {booking.status === 'approved' ? 'Confirmada' : booking.status === 'pending' ? 'Pendiente' : 'Cancelada'}
                    </div>
                </div>
                ...
                {!isPast && booking.status !== 'cancelled' && (
                    <div className="pt-4 border-t border-slate-800 flex justify-end gap-3">
                        <button className="text-xs text-slate-400 hover:text-white transition-colors">Cancelar Cita</button>
                        <button className="text-xs bg-slate-800 hover:bg-slate-700 text-white px-3 py-1.5 rounded transition-colors">Modificar</button>
                    </div>
                )}
            </div>
        </div>
    ...
    return (
        <div className="space-y-8">
            ...
            {pastBookings.length > 0 && (
                <div>
                    <h3 className="text-lg font-bold text-slate-400 mb-4 uppercase tracking-wider text-sm">Historial</h3>
