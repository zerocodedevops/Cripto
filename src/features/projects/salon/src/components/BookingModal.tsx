	...
	useEffect(() => {
		async function fetchData() {
			try {
				...
				const newCatalog: ServiceCategory[] = Object.keys(grouped).map(
					(key) => ({
						id: key,
			...
			>
				...
				<div className="p-6 border-b border-white/10 bg-[#BF953F]/5">
					<div className="flex justify-between items-start mb-4">
						<div>
							<h2 className="font-heading text-2xl text-[#FCF6BA]">
								Reserva tu Cita
							</h2>
							...
							>
								...
								<div className="space-y-4">
									{catalog.map((category) => (
										<div
											key={category.id}
											className="border border-white/5 rounded-sm overflow-hidden bg-white/[0.02]"
										>
											<button
											...
											>
												<span className="font-bold text-[#BF953F] tracking-wider text-sm">
													{category.title}
												</span>
												<ChevronDown
													size={16}
																...
																>
																	<div className="flex-1 pr-4">
																		<div className="flex items-center gap-2 mb-1">
																			<h4
																				className={`font-medium text-sm ${selectedServices.includes(service.id) ? "text-[#FCF6BA]" : "text-neutral-300"}`}
																			>
																				{service.title}
																			</h4>
																			<span className="text-[10px] px-1.5 py-0.5 rounded bg-white/5 text-neutral-500 border border-white/5 whitespace-nowrap">
																				{service.duration}
																			</span>
																		</div>
																		{service.description && (
																	...
																	<div className="text-right">
																		<span className="text-[#BF953F] font-bold text-sm block">
																			{service.price}€
																		</span>
																		<div
																			className={`w-5 h-5 rounded-full border flex items-center justify-center transition-all mt-2 ml-auto ${selectedServices.includes(service.id) ? "bg-[#BF953F] border-[#BF953F]" : "border-white/20 group-hover:border-white/40"}`}
																		>
						...
						{step === 2 && (
							...
							>
								...
								>
									...
									<div>
										<h4 className="text-[#FCF6BA] font-bold">
									...
									{selectedStylist === null && (
										<Check className="ml-auto text-[#BF953F]" />
									)}
								</button>
									...
									>
										<div className="w-12 h-12 rounded-full bg-neutral-800 flex items-center justify-center border border-white/10 overflow-hidden">
											{stylist.image ? (
												...
												<span className="text-lg font-bold text-[#BF953F]">
													{stylist.name[0]}
												</span>
											)}
										</div>
							...
							>
								...
								<div>
									<h3 className="text-[#FCF6BA] font-heading mb-4 flex items-center gap-2">
										<Calendar size={18} className="text-[#BF953F]" /> Selecciona
										el Día
									</h3>
								...
								{selectedDate && (
									...
									>
										<h3 className="text-[#FCF6BA] font-heading mb-4 flex items-center gap-2">
											<Clock size={18} className="text-[#BF953F]" /> Selecciona
											la Hora
										</h3>
