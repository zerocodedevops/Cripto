			...
			<div className="flex-1 overflow-y-auto overflow-x-hidden p-6 md:p-12 lg:p-20">
				<div className="max-w-2xl mx-auto w-full h-full flex flex-col justify-center">
					<AnimatePresence mode="wait">
						{step === 1 && <CategoryStep key="step1" categories={categories} />}
						{step === 2 && <ServiceStep key="step2" services={services} />}
						{step === 3 && <StaffStep key="step3" staff={staff} />}
						{step === 4 && <DateStep key="step4" />}
						{step === 5 && <ConfirmationStep key="step5" />}
					</AnimatePresence>
				</div>
			</div>
		</div>
