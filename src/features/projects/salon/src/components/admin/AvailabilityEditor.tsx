...
export default function AvailabilityEditor({
	...
	const updateDay = (dayId: number, start: string, end: string) => {
		...
		if (existing) {
			setSchedule(
				schedule.map((s) =>
					s.dayOfWeek === dayId ? { ...s, startTime: start, endTime: end } : s,
				),
			);
		} else {
			setSchedule([
				...schedule,
				{ dayOfWeek: dayId, startTime: start, endTime: end },
			]);
		}
	};
			...
			<div className="space-y-4">
				{DAYS.map((day) => {
					...
					return (
						<div
							key={day.id}
							className="flex items-center gap-4 bg-white/5 p-3 rounded-sm"
						>
								...
								<>
									<div className="flex items-center gap-2 text-white font-mono">
										...
										<input
											...
											onChange={(e) =>
												updateDay(day.id, e.target.value, slot.endTime)
											}
										...
										<input
											...
											value={slot.endTime}
											onChange={(e) =>
												updateDay(day.id, slot.startTime, e.target.value)
											}
											className="bg-transparent border-b border-white/20 focus:border-gold-500 outline-none w-20 text-center"
										/>
									</div>
