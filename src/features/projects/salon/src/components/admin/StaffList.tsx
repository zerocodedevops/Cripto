	...
	return (
		<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
			{staffList.map((member) => (
				<div
					key={member.id}
					className="bg-neutral-800 border border-white/10 rounded-sm p-6 space-y-4"
				>
					<div className="flex justify-between items-start">
						<div className="flex items-center gap-4">
							...
							<div>
								<h3
									className="text-lg font-bold text-white max-w-[150px] truncate"
									title={member.name}
								>
									{member.name}
								</h3>
								<Link
									...
									className="text-xs text-gold-500 hover:text-white underline underline-offset-4 decoration-gold-500/30 hover:decoration-white/50 transition-all"
								>
									Gestionar Horario
								</Link>
							</div>
					...
					<div className="border-t border-white/5 pt-4">
						...
						<div className="flex flex-wrap gap-2">
							{member.services.map((s: any) => (
								<span
									key={s.id}
									className="text-xs px-2 py-1 bg-white/5 rounded-sm text-neutral-300 border border-white/5"
								>
									{s.name}
								</span>
							))}
