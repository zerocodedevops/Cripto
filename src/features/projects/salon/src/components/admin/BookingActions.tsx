                ...
                {isOpen && (
                    ...
                    >
                        <div className="p-1 space-y-1">
                            <button onClick={() => { setIsEditOpen(true); setIsOpen(false); }} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-left text-[#FCF6BA] hover:bg-white/5 rounded-sm">
                                <Edit size={14} /> Editar
                            </button>
                            <div className="h-px bg-white/10 my-1" />
                            {currentStatus !== 'CONFIRMED' && (
                                <button onClick={() => handleStatus('CONFIRMED')} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-left text-green-500 hover:bg-white/5 rounded-sm">
                                    <CheckCircle size={14} /> Confirmar
                                </button>
                            )}
                            {currentStatus !== 'CANCELLED' && (
                                <button onClick={() => handleStatus('CANCELLED')} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-left text-yellow-500 hover:bg-white/5 rounded-sm">
                                    <XCircle size={14} /> Cancelar
                                </button>
                            )}
                            <div className="h-px bg-white/10 my-1" />
                            <button onClick={handleDelete} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-left text-red-500 hover:bg-white/5 rounded-sm">
