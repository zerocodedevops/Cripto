...
import {
	createBooking,
	getAvailableSlots,
} from "@salon/features/booking/actions";
import { useEffect, useState } from "react";

export default function BookingDebugger({
	services,
	staff,
}: Readonly<{ services: any[]; staff: any[] }>) {
					...
					<CardContent className="space-y-6">
						<div className="space-y-2">
							...
							>
								<SelectTrigger
									id="category-select"
									className="border-neutral-700 bg-neutral-950 text-neutral-200 focus:ring-gold-500"
								>
								...
								<SelectContent className="bg-neutral-900 border-neutral-800 text-neutral-200">
									{categories.map((cat) => (
										<SelectItem
											key={cat}
											value={cat}
											className="focus:bg-neutral-800 focus:text-gold-500"
										>
											{cat}
										</SelectItem>
									))}
						...
						<Button
							...
							className="w-full bg-gold-500 text-black hover:bg-gold-400 font-bold transition-all"
						>
							Check Slots
						</Button>
					</CardContent>
				...
				<Card className="bg-neutral-900 border-neutral-800 text-neutral-100">
					...
					<CardContent className="space-y-4">
						<div className="bg-black/50 border border-neutral-800 p-4 rounded h-40 overflow-auto text-xs font-mono text-gold-200/80">
							{slots.length === 0
								? "No slots available or not checked yet."
								: JSON.stringify(slots, null, 2)}
						</div>
						<Button
							onClick={handleBook}
							variant="outline"
							className="w-full border-gold-500 text-gold-500 hover:bg-gold-500 hover:text-black transition-colors uppercase tracking-widest text-xs font-bold"
						>
