  ...
  return (
    <main className="relative min-h-screen flex flex-col justify-between bg-neutral-950 overflow-hidden selection:bg-[#BF953F] selection:text-black">
      ...
      <nav className="relative z-10 w-full p-8 flex justify-between items-center border-b border-white/5 backdrop-blur-sm">
        <div className="text-xs tracking-[0.3em] font-bold text-[#FCF6BA] uppercase">{t('branding')}</div>
        <a href="/auth/login" className="text-xs tracking-[0.2em] font-medium text-neutral-400 hover:text-[#FCF6BA] transition-colors uppercase">
          {t('login')}
        </a>
      </nav>
      ...
      <section className="relative z-10 flex-1 flex flex-col items-center justify-center p-6 text-center space-y-12">
        ...
        <div className="flex flex-col sm:flex-row gap-8 items-center pt-8">
          <Link href={`/${locale}/book`} className="group relative px-8 py-4 bg-transparent overflow-hidden">
            ...
            <span className="relative text-xs tracking-widest text-[#BF953F] uppercase">_01</span>
          </Link>

          <a href="/auth/login" className="group relative px-8 py-4 bg-transparent">
