...
import { ArrowLeft, Bell, LogOut, Search, User } from "lucide-react";
...
import type { DashboardData, DateRange, Segment } from "../types";
import {
	premiumColors,
	premiumEffects,
	premiumLayout,
...
export default function DashboardPage() {
	...
	useEffect(() => {
		...
		const interval = setInterval(() => {
			setLiveData((prev) => {
				...
				const newRevenue =
					Math.random() > 0.7 ? Math.floor(Math.random() * 150) : 0;
				...
				next.kpi.activeUsers.value = (
					currentUsers + userChange
				).toLocaleString();

				// Update Revenue (only up)
				if (newRevenue > 0) {
					const currentRev = Number.parseFloat(
						String(next.kpi.totalRevenue.value)
							.replaceAll(",", "")
							.replace("€", ""),
					); // Handle format
					next.kpi.totalRevenue.value = (
						currentRev + newRevenue
					).toLocaleString();
	...
	const handleLogin = (username: string) => {
		const user = {
			name: username,
			email: `${username.toLowerCase()}@fitn.net`,
		};
		setCurrentUser(user);
		...
		return (
			<div className="flex items-center justify-center min-h-screen bg-[#080808]">
				<div className="text-center p-12 rounded-[32px] border border-white/10 glass">
					...
					<p className="text-sm opacity-60 text-[#8E8E93]">
						{t("failedToLoad")}
					</p>
				</div>
			</div>
		...
		>
			...
			<div className="fixed inset-0 overflow-hidden pointer-events-none">
				<div
					...
					style={{ backgroundColor: premiumColors.brand.primary }}
				></div>
				<div
					className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full opacity-10 blur-[120px]"
					style={{ backgroundColor: premiumColors.brand.secondary }}
				></div>
			</div>
				...
				<header className="flex items-center justify-between mb-10">
					<div className="flex items-center gap-12">
						...
						>
							...
							<span className="text-xs font-semibold tracking-wide uppercase opacity-70 group-hover:opacity-100">
								{t("backToPortfolio")}
							</span>
						</Link>
						...
						<div className="hidden md:flex items-center gap-8 text-sm font-medium">
							{(
								["Overview", "Analytics", "Finance", "Strategy"] as TabType[]
							).map((tab) => (
								<button
									key={tab}
						...
						<div className="flex items-center gap-3">
							<div className="relative">
								...
								>
									...
									<div
										className="absolute top-2 right-2 w-2 h-2 rounded-full border-2 border-[#080808]"
										style={{ backgroundColor: premiumColors.brand.tertiary }}
									></div>
								</button>
							...
							<div className="flex items-center gap-3 pl-4 border-l border-white/10">
								<div className="text-right hidden lg:block">
									...
									<p className="text-[10px] opacity-40">
										{currentUser?.email || "initializing..."}
									</p>
								</div>
								<div className="group relative">
									...
									>
										<div
											className={`w-2 h-2 rounded-full ${isLiveMode ? "bg-red-500 animate-pulse" : "bg-white/50"}`}
										/>
										<span
											className={`text-xs font-bold tracking-widest uppercase ${isLiveMode ? "text-red-400" : "opacity-50"}`}
										>
											Live
										</span>
									</button>
									...
									{shouldShowChart("deviceDist") && (
										<div className="min-h-[400px]">
											<PremiumConversionPieChart
												...
												isLoading={isLoading}
												onSliceClick={(label, val) =>
													handleDrillDown("DEVICE_ENTITY", label, `${val}%`)
												}
											/>
										</div>
									)}
							...
							<div className="xl:col-span-4 flex flex-col gap-6 min-h-0">
								<AnimatePresence mode="wait">
									{drillDownData ? (
										...
										>
											...
											>
												...
												<div className="flex-1 flex flex-col justify-center text-center">
													<p className="text-[10px] uppercase tracking-[0.3em] mb-4 opacity-50">
														Signal Extraction
													</p>
													<h4
														className="text-6xl font-black mb-8"
														style={{ color: premiumColors.accent.white }}
													>
														{drillDownData.value}
													</h4>
													...
													<div className="flex justify-center">
														<div className="px-6 py-2 rounded-full border border-white/10 bg-white/5 text-[10px] font-bold uppercase tracking-widest">
															Status:{" "}
															<span
																style={{ color: premiumColors.brand.secondary }}
															>
																Analyzed
															</span>
														</div>
												...
												<div className="mt-12 p-4 rounded-3xl bg-white/5 border border-white/5">
													<p className="text-xs opacity-60 leading-relaxed italic">
														"The analyzed segment shows high-performance
														correlation with current market trends. Priority
														extraction complete."
													</p>
												</div>
											</div>
										...
										<div
											className="h-full rounded-[32px] border border-white/5 bg-white/5 flex flex-col items-center justify-center p-8 text-center"
											style={premiumEffects.glassmorphism}
										>
											...
											<p className="opacity-30 text-xs leading-relaxed">
												Select a data point from any chart to view detailed
												drill-down metrics here.
											</p>
										</div>
									)}
								</AnimatePresence>
