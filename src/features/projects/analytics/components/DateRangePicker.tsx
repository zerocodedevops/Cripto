...
export function DateRangePicker({ onRangeSelect, currentRange }: DateRangePickerProps) {
  ...
  return (
    <div className="relative">
      ...
      >
        <Calendar className="w-4 h-4 opacity-100" style={{ color: premiumColors.brand.primary }} />
        <span className="tracking-wide">
          {formatDate(currentRange.start)} - {formatDate(currentRange.end)}
        </span>
        <ChevronDown className={`w-3 h-3 transition-transform duration-300 opacity-40 ${isOpen ? 'rotate-180' : ''}`} />
      </button>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="p-2">
              <p 
                className="text-[10px] font-bold tracking-widest mb-3 uppercase opacity-40"
              >
                {t('quickSelect')}
              </p>
