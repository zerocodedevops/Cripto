...
const generateInsights = (
	...
	const upTrends = Object.values(data.kpi).filter(
		(kpi) => kpi.trend === "up",
	).length;
	const downTrends = Object.values(data.kpi).filter(
		(kpi) => kpi.trend === "down",
	).length;
	...
	const highestDropOff = Math.max(
		...funnelData.map((step) => step.dropOff || 0),
	);
	const criticalStep = funnelData.find(
		(step) => step.dropOff === highestDropOff,
	);
	...
	const mobileRevenue = data.revenueByDevice.find((d) =>
	...
	const avgRecent =
		recentSales.reduce((sum, item) => sum + item.sales, 0) / recentSales.length;
	const olderSales = data.salesTrend.slice(0, 3);
	const avgOlder =
		olderSales.reduce((sum, item) => sum + item.sales, 0) / olderSales.length;
			...
			>
				<div className="flex items-center gap-4 mb-6">
					<div
						...
						style={{ backgroundColor: premiumColors.brand.quaternary }}
					></div>
					<h3
