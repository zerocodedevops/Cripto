...
const MOCK_NOTIFICATIONS: Notification[] = [
	...
	},
	{
		id: "2",
		type: "warning",
		title: "Ad spend spikes in EU North",
		time: "15m ago",
		read: false,
	},
	{
		id: "3",
		type: "alert",
		title: "Unusual drop-off in Checkout",
		time: "1h ago",
		read: true,
	},
	{
		id: "4",
		type: "info",
		title: "New weekly report is ready",
		time: "3h ago",
		read: true,
	},
];
...
export function NotificationDropdown({
	...
	const getIcon = (type: string) => {
		switch (type) {
			case "success":
				return <CheckCircle2 className="w-3 h-3 text-sage-400" />;
			case "warning":
				return <TrendingUp className="w-3 h-3 text-peach-400" />;
			case "alert":
				return <AlertTriangle className="w-3 h-3 text-coral-400" />;
			default:
				return <Info className="w-3 h-3 text-lavender-400" />;
		}
	};
