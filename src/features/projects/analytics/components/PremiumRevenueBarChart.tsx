		...
		>
			...
			<div className="space-y-1">
				...
				{data.dropOff > 0 && (
					<p
						className="text-[10px]"
						style={{ color: premiumColors.brand.tertiary }}
					>
...
export function PremiumRevenueBarChart({
	data,
	isLoading,
	onBarClick,
}: PremiumRevenueBarChartProps) {
	const { t } = useLanguage();
						...
						>
							{data.map((_entry, index) => (
								<motion.rect
									...
									fill={
										PREMIUM_FUNNEL_COLORS[index % PREMIUM_FUNNEL_COLORS.length]
									}
								/>
							))}
						</Bar>
