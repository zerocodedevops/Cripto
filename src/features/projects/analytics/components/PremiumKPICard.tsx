...
export function PremiumKPICard({ kpi, sparklineData = [], isLoading }: PremiumKPICardProps) {
  const { t } = useLanguage();
  ...
  return (
    ...
    >
      <div className="relative z-10">
        ...
        >
          <span className="text-2xl font-bold tracking-tighter">{kpi.value}€</span>
        </h3>
      </div>
      ...
      {sparklineData.length > 0 && (
        <div className="mt-4 h-12 relative w-full overflow-hidden">
          <svg className="w-full h-full" preserveAspectRatio="none" viewBox="0 0 100 100">
            <defs>
              <linearGradient id={`premium-gradient-${kpi.title}`} x1="0%" y1="0%" x2="0%" y2="100%">
      ...
      <div className="mt-3 flex items-center justify-between text-[10px]" style={{ color: premiumColors.accent.grey }}>
         <span className="font-medium uppercase tracking-widest opacity-50">{t('live')}</span>
