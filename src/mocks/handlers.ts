...
import { DashboardData } from '../features/projects/analytics/types';
...
export const handlers = [
  http.get('/api/analytics/dashboard', ({ request }) => {
    ...
    console.log(`Generating mock data for range: ${range}, segment: ${segment}`);
    ...
    const scale = (valStr: string | number, mul: number, isPercent = false): string => {
       if (typeof valStr === 'number') return (valStr * mul).toLocaleString();
       const num = Number.parseFloat(valStr.replaceAll(',', '').replaceAll('%', ''));
    ...
    const mockData: DashboardData = {
      ...
      salesTrend: (() => {
        ...
        return Array.from({ length: trendLength }, (_, i) => {
         ...
         const baseSales = getSegmentValue(1000, 2000, 3000) * multiplier / salesDivisor;
         const baseRevenue = getSegmentValue(2500, 5000, 7500) * multiplier / revenueDivisor;
      ...
      revenueByDevice: (() => {
        ...
        return [
          ...
          { device: 'Tablet', value: segment === 'all' ? 20 : 0, color: '#10b981' },
        ];
      })(),
      conversionFunnel: [
        { name: 'Visitas', count: Math.floor(getSegmentValue(5000, 7000, 12000) * multiplier), dropOff: 0 },
        { name: 'Carrito', count: Math.floor(getSegmentValue(2000, 2500, 4500) * multiplier), dropOff: 60 },
        { name: 'Checkout', count: Math.floor(getSegmentValue(800, 1000, 1800) * multiplier), dropOff: 60 },
        { name: 'Compra', count: Math.floor(getSegmentValue(400, 550, 950) * multiplier), dropOff: 50 },
      ],
    };
  ...
  http.get('https://api.coingecko.com/api/v3/coins/markets', ({ request }) => {
    ...
    const mockCryptoData = Array.from({ length: Math.min(perPage, 50) }, (_, i) => {
      const crypto = cryptoList[i] || { id: `coin-${i}`, symbol: `coin${i}`, name: `Coin ${i}` };
      ...
      return {
        ...
        current_price,
        market_cap: [1850000000000, 402000000000, 140000000000, 95000000000, 89000000000, 43000000000, 33000000000, 50000000000, 20000000000, 13000000000][i] || 1000000000 * (100 - i),
        market_cap_rank: i + 1,
        fully_diluted_valuation: null,
        total_volume: [45000000000, 22000000000, 95000000000, 1800000000, 4200000000, 7500000000, 750000000, 2800000000, 1200000000, 485000000][i] || 1000000,
        ...
        sparkline_in_7d: {
          price: Array.from({ length: 168 }, (_, j) => basePrice + (Math.random() - 0.5) * (basePrice * 0.05))
        },
        price_change_percentage_7d_in_currency: 2.34 + (Math.random() - 0.5) * 3
      };
    });
  ...
  http.get('https://api.coingecko.com/api/v3/global', () => {
    return HttpResponse.json({
      data: {
        ...
        markets: 1184,
        total_market_cap: { usd: 3220000000000 + (Math.random() - 0.5) * 100000000000 },
        total_volume: { usd: 134140000000 + (Math.random() - 0.5) * 10000000000 },
  ...
  http.get('https://api.coingecko.com/api/v3/search/trending', () => {
    const trendingCoins = [
      ...
      { id: 'avalanche', name: 'Avalanche', symbol: 'AVAX', market_cap_rank: 12 },
  ...
  http.get('https://api.coingecko.com/api/v3/coins/:id/market_chart', ({ request }) => {
    const url = new URL(request.url);
    ...
    return HttpResponse.json({
      ...
      total_volumes: prices.map(([time, price]) => [time, price * 450000 * (0.8 + Math.random() * 0.4)])
    });
  }),
  ...
  http.get('https://api.coingecko.com/api/v3/*', ({ request }) => {
    console.warn('⚠️ [MSW] Unhandled CoinGecko request caught by wildcard:', request.url);
    return HttpResponse.json({});
  })
];
