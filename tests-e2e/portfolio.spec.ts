...
test.describe("Portfolio E2E Strict Tests", () => {
	...
	test("should verify strict accessibility and layout presence", async ({
		page,
	}) => {
		// Hero Section
		await expect(page.locator("#hero")).toBeVisible();
		await expect(
			page.getByRole("heading", { level: 1, name: /Hola, soy/i }),
		).toBeVisible();
	...
	test("should have working social links", async ({ page }) => {
		...
		const githubLink = page
			.locator('#hero a[href="https://github.com/zerocodedevops"]')
			.first();
		...
		const linkedinLink = page
			.locator('#hero a[href="https://www.linkedin.com/in/zerocode-devops"]')
			.first();
